...
def check_tally_consistency(content: str) -> list[str]:
    """헤더 'UF N개' · 요약표 행 수 · 본문 블록 수 · 소유자 분포의 대조.
    2026-07-27 추가 — 증보 후 집계 미갱신(50 vs 55)·'109블록' 오보 재발 방지."""
    failures = []
    block_count = len(re.findall(r"^- UF-ID: UF-\d{2}-\d{2}", content, re.MULTILINE))

    header_match = re.search(r"UF\s+(\d+)개", content)
    if header_match and int(header_match.group(1)) != block_count:
        failures.append(
            f"header declares 'UF {header_match.group(1)}개' but body has {block_count} blocks"
        )

    table_rows = len(re.findall(r"^\|\s*UF-\d{2}-\d{2}\s*\|", content, re.MULTILINE))
    if table_rows and table_rows != block_count:
        failures.append(
            f"summary table has {table_rows} UF rows but body has {block_count} blocks"
        )
...
def main():
    ...
    if not all_issues:
        # 블록 수는 반드시 블록 헤더(^- UF-ID:)로 센다.
        # findall("UF-\\d{2}-\\d{2}") 전체 출현 카운트는 요약표·상호참조까지 세는 오보였음(2026-07-27 수정).

...
- `if_list.md`, `if_decomposition.md`, `uf.md`, `uf_split/`, `uf_if_coverage_review.md`
...
### Rule 16 — Directory for Implementation & Audit Reports
...
> `reports/impl/` (uf-implementor, if-integrator), `reports/eval/` (eval-runner, exp-runner), `reports/physics/` (sim-physics-auditor), `reports/rag/` (rag-data-quality), `reports/gpu/` (gpu-hpc-guard).
